...
export default async function handler(req, res) {
  ...
  // 4. MEMORY & IDENTITY (No aggressive recall & isolated study context)
...
- User new hai, naam unknown hai. Baaton-baaton mein casual tarike se naam poocho aur ek cool nickname do.`;
  ...
  switch (mode) {
    ...
    case 'gossip':
      ...
      break;
    case 'study':
      modePrompt = "MODE: STUDY BUDDY (Academic Only). Factual direct answer do. Math/code/logic ko clean steps mein breakdown karo without forced analogies. Max 3-4 short sentences.";
      break;
    case 'dilse':
    default:
  ...
  // 7. SYSTEM PROMPT (Strictly Male, Crisp WhatsApp Tone)
...
OUTPUT FORMAT (JSON ONLY):
{
  ...
  // 8. ACTIVE MODEL WATERFALL FALLBACK ENGINE
  // Priority: Fast Lite models first (immune to 503 spikes), followed by previews
  ...
  for (const model of activeWaterfall) {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 2500); // 2.5s strict timeout per candidate
    ...
    try {
      ...
      // If Google throws 503, 429, or 404, silently jump to next active model
      if (!geminiRes.ok) {
        lastFailReason = `[${model} Status ${geminiRes.status}]: ` + await geminiRes.text();
        continue;
      }
      ...
      clearTimeout(timeoutId);
      lastFailReason = `[${model} Error]: ` + err.message;
      continue;
    }
  }

  // Triggered only if every active model in the chain fails
  return res.status(200).json({
    text: "[Google All Models Busy]: " + lastFailReason,
    voiceEnabled: updatedVoiceEnabled,
    extractedName: name,
    nickname: nickname,
    memoryTopic: lastContext
