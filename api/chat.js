...
export default async function handler(req, res) {
  ...
  // 5. LOCKED MEMORY (SINGLE NAME ASK)
...
OUTPUT FORMAT (JSON ONLY):
{
  ...
  try {
    // 8. FAST AUTO-DISCOVERY (Finds the active model for your key in 150ms)
    let activeModel = cachedModel;
    ...
    if (!activeModel) {
      try {
        ...
        if (listRes.ok) {
          ...
          const best = available.find(m => m.name.includes('2.0-flash'))
            || available.find(m => m.name.includes('flash'))
            || available[0];

          if (best && best.name) {
            activeModel = best.name.replace('models/', '');
            cachedModel = activeModel;
          }
        }
      } catch (e) {
        // Fallback default if listing fails
      }
    }

    if (!activeModel) activeModel = 'gemini-2.0-flash';

    // 9. PREPARE MULTIMODAL PAYLOAD
    const userParts = [];
    if (imageBase64) {
    ...
    // 10. SINGLE FAST API CALL (< 1.5 seconds)
    ...
    return res.status(200).json({
      text: "Arey, thoda sa network atak gaya tha! Phir se bolo na?",
      voiceEnabled: updatedVoiceEnabled,
      extractedName: name,
      nickname: nickname,
      memoryTopic: lastContext
