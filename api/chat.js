...
OUTPUT FORMAT (JSON ONLY):
{
  ...
  // 8. DIRECT TARGET: gemini-3.6-flash (Zero loops, 7-second safety abort)
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(200).json({
        text: `[Google 3.6-Flash Error ${geminiRes.status}]: ${errDetail}`,
        voiceEnabled: updatedVoiceEnabled,
        extractedName: name,
        nickname: nickname,
        memoryTopic: lastContext
    ...
    clearTimeout(timeoutId);
    return res.status(200).json({
      text: `[Execution Notice]: ${err.name === 'AbortError' ? 'Google API response took > 7s' : err.message}`,
      voiceEnabled: updatedVoiceEnabled,
      extractedName: name,
      nickname: nickname,
      memoryTopic: lastContext
