...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json({ text: "[Error 405]: Only POST method allowed" });
  }
  ...
  if (isFirstTime) {
    return res.status(200).json({
      text: "welcome to dilseSuno. Main piku hoon, tumhara apna AI dost. Jo bhi chal raha ho mann mein—stress, bakchodi ya koi baat—bina filter ke bol sakte ho.",
      voiceEnabled: true,
      extractedName: "",
      nickname: "",
      memoryTopic: ""
  ...
  if (crisisKeywords.some(k => lowerMsg.includes(k))) {
    return res.status(200).json({
      text: "Bhai sun, main ek AI hoon aur ye cheez akele handle nahi hogi. Plz helpline par baat kar: Tele-MANAS (14416) ya KIRAN (1800-599-0019). Tu akele nahi hai.",
      isCrisis: true,
...
- ANTI-AGGRESSIVE RECALL: Past context ("${effectiveContext}") ko achanak pehle sentence mein zabardasti mat chhedo jab tak user khud wo topic na uthaye. Casual WhatsApp friend ki tarah reply do.`;
  } else {
    memoryInstruction = `
- User new hai, naam unknown hai. Baaton-baaton mein casual tarike se naam poocho aur ek cool nickname do.`;
  ...
  switch (mode) {
    ...
    case 'gossip':
      ...
      break;
    case 'study':
      modePrompt = "MODE: STUDY BUDDY (Academic Only). Factual direct answer do. Math/code/logic ko clean steps mein breakdown karo without forced cricket/chai analogies. Max 3-4 short sentences.";
      break;
    case 'dilse':
    default:
...
1. GENDER: You are 100% MALE. Always use male first-person Hindi verbs ("main samajhta hoon", "bol raha hoon", "sun raha hoon"). NEVER use female verbs ("samajhti hoon", "bataungi").
2. BANNED SCRIPTS: NEVER say "Main samajh sakta hoon ki ye waqt kitna mushkil hai", "Aapka dard gehra hai", "Kya share karoge". Talk like a genuine friend on WhatsApp.
3. NO FORCED QUESTIONS: Do NOT append survey or interview questions to every reply.
4. NO ENGLISH SWITCH: Stay naturally in casual Hinglish. Do NOT switch to corporate English.
...
OUTPUT FORMAT (JSON ONLY):
{
  ...
  "nickname": "Nickname if assigned, else '${nickname}'",
  ...
  if (!apiKey) {
    return res.status(200).json({
      text: "[Backend Error]: Vercel Settings -> Environment Variables me GEMINI_API_KEY set nahi hai!",
  ...
  // 8. AUTO-DISCOVER WORKING GOOGLE MODEL DYNAMICALLY (150ms)
  let targetModel = "";
  try {
    const listRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${apiKey}`);
    if (!listRes.ok) {
      const errDetail = await listRes.text();
      return res.status(200).json({
        text: `[Google ListModels Error ${listRes.status}]: ${errDetail}`,
        voiceEnabled: updatedVoiceEnabled,
        extractedName: name,
        nickname: nickname,
        memoryTopic: lastContext
    ...
    if (!live.length) {
      return res.status(200).json({
        text: "[Google Account Error]: Tumhari is API Key par koi bhi generateContent model activate nahi hai!",
        voiceEnabled: updatedVoiceEnabled,
        extractedName: name,
        nickname: nickname,
        memoryTopic: lastContext
    ...
    // Auto-pick the active flash model
    targetModel = live.find(m => m.includes('flash')) || live[0];

  } catch (err) {
    return res.status(200).json({
      text: `[Google Connection Failed]: ${err.message}`,
      voiceEnabled: updatedVoiceEnabled,
      extractedName: name,
      nickname: nickname,
      memoryTopic: lastContext
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(200).json({
        text: `[Google API Error on ${targetModel} - Status ${geminiRes.status}]: ${errBody}`,
        voiceEnabled: updatedVoiceEnabled,
        extractedName: name,
        nickname: nickname,
        memoryTopic: lastContext
    ...
    return res.status(200).json({
      text: parsed.text || "Bhai samajh nahi aaya, firse bol na?",
      voiceEnabled: updatedVoiceEnabled,
    ...
    return res.status(200).json({
      text: `[Backend Runtime Error]: ${err.message}`,
      voiceEnabled: updatedVoiceEnabled,
      extractedName: name,
      nickname: nickname,
      memoryTopic: lastContext
