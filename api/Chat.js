...
export default async function handler(req, res) {
  ...
  if (isFirstTime) {
    return res.status(200).json({
      text: "welcome to dilseSuno. Main piku hoon, tumhara apna AI dost. Jo bhi chal raha ho dimaag mein—stress, bakchodi ya koi baat—bina filter ke bol sakte ho.",
      voiceEnabled: true,
      extractedName: "",
      nickname: "",
      memoryTopic: ""
  ...
  // 3. DYNAMIC VOICE TOGGLE KEYWORDS
...
- User known hai. DOBARA NAAM KABHI MAT POOCHNA.
- ANTI-AGGRESSIVE RECALL: Past context ("${effectiveContext}") ko achanak pehle sentence mein zabardasti mat chhedo jab tak user khud wo topic na uthaye. Casual WhatsApp friend ki tarah reply do.`;
    ...
    memoryInstruction = `
- User new hai, naam unknown hai. Baaton-baaton mein casual tarike se naam poocho aur ek cute/cool nickname do.`;
  }
  ...
  switch (mode) {
    case 'bhai':
      modePrompt = "MODE: REAL TALK (Bada Bhai). Practical reality check, no gyan, no sugarcoating. 1-2 short lines.";
      break;
    case 'hype':
...
OUTPUT FORMAT (JSON ONLY):
{
  ...
  // Screen verified active models list
  ...
  for (const model of candidateModels) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 4500); // 4.5s per candidate to prevent Vercel 500 timeout
